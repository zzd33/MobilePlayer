package com.bobo.mobileplayer.service;
...
import com.bobo.mobileplayer.base.NotificationsUtils;
...
//            notifyChange(OPENAUDIO);
    ...
    private void start(){
        ...
        if (!b){//用户没有开启通知栏的权限提示用户开启
            Toast.makeText(this,"波波提示：设置-通知与状态栏开启权限",Toast.LENGTH_LONG).show();
        }
        //---------------------------leon-↑-----------------------------------

        //当播放歌曲的时候，在状态栏显示正在播放，点击的时候可以进入音乐播放页面
    ...
    private void openNextAudio() {
        ...
        if (playmode == MusicPlayerService.REPEAT_NORMAL){
            if (position < mediaItems.size()){
                ...
                pause();// 这里是leon 增加 解决全部播放最后一首歌曲状态不对的bug
